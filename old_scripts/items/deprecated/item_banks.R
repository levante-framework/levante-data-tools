# export item bank googlesheets to CSVs for import into airtable
...
# read in all corpus sheets
# treat all columns as character (due to type mismatches between sheets)
corpus_list <- corpus_sheets |> set_names() |>
  map(\(s) read_sheet(item_bank_id, sheet = s, na = c("", "NA"), col_types = "c"))
...
# write_csv(corpus_df, here("item_banks.csv"), na = "")
